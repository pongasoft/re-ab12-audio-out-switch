...
for k, output in pairs(outputs) do
  ...
  -- back
  output.audioJackLeft = computeAbsolute(audioJack, { outputSpacingX * (back["c"] - 1), 0 })

  -- front
  local front = output["f"]
  output["label"] =  computeAbsolute({banks["columns"][front["c"]], banks["rows"][front["r"]]}, offsets[front["o"]]["label"])
  output["switch"] = computeAbsolute({banks["columns"][front["c"]], banks["rows"][front["r"]]}, offsets[front["o"]]["switch"])
  output["volume"] = computeAbsolute({banks["columns"][front["c"]], banks["rows"][front["r"]]}, offsets[front["o"]]["volume"])

  -- folded front
  output["switchFF"] = computeAbsolute(ffSwitch, { foldedFrontSpacing * output["ff"], 0 })
...
for k, output in pairs(outputs) do
  ...
  back[#back + 1] = {
    -- audio/cv outputs
    offset = output.audioJackLeft,
    ["audioOutputStereoPairLeft" .. k] = {
      offset = { 0, 0 },
      { path = "Cable_Attachment_Audio_01_1frames" }
    },
    ["audioOutputStereoPairRight" .. k] = {
      offset = audioJackRightOffset,
      { path = "Cable_Attachment_Audio_01_1frames" }
    },
    ["CVInGate" .. k] = {
